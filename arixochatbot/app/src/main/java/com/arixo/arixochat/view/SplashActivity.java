...
import com.arixo.arixochat.Constants;
...
public class SplashActivity extends Activity implements SplashADListener {
    ...
    private boolean mNeedStartDemoList = true;
     ...
     **/
    private static final int MIN_SPLASH_TIME_NO_AD = 5000;
    /**
     * 记录拉取广告的时间
     */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContainer = (ViewGroup) this.findViewById(R.id.splash_container);
        mSplashHolder = (ImageView) findViewById(R.id.splash_holder);
        boolean needLogo = getIntent().getBooleanExtra("need_logo", true);
        mNeedStartDemoList = getIntent().getBooleanExtra("need_start_demo_list", true);
        if (!needLogo) {
    ...
    private void checkAndRequestPermission() {
        ...
        if (lackedPermission.size() == 0) {
            ...
            Log.d(TAG, "checkAndRequestPermission: PosId: " + posId);
            fetchSplashAD(this, mContainer, posId, this, 0);
        } else {
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == 1024 && hasAllPermissionsGranted(grantResults)) {
            fetchSplashAD(this, mContainer, getPosId(), this, 0);
        } else {
    ...
    private void fetchSplashAD(Activity activity, ViewGroup adContainer, String posId, SplashADListener adListener, int fetchDelay) {
        ...
        mSplashAD = new SplashAD(activity, posId, adListener, fetchDelay);
    ...
    public void onNoAD(AdError error) {
        ...
        long shouldDelayMills = alreadyDelayMills > MIN_SPLASH_TIME_NO_AD ? 0 : MIN_SPLASH_TIME_NO_AD
                - alreadyDelayMills;//为防止加载广告失败后立刻跳离开屏可能造成的视觉上类似于"闪退"的情况，根据设置的minSplashTimeWhenNoAD
        // 计算出还需要延时多久
        mHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                if (mNeedStartDemoList) {
